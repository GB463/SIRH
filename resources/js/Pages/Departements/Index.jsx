...
export default function DepartementsIndex() {
    ...
    function handleSubmit(e) {
        e.preventDefault();
        if (editingDep) {
            router.put("/departements/" + editingDep.id, form, { onSuccess: function () { setShowModal(false); } });
        } else {
            router.post("/departements", form, { onSuccess: function () { setShowModal(false); } });
        }
    }
                ...
                <table className="w-full">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            {isAdmin && <th className="px-6 py-3 text-left text-sm font-semibold text-gray-600">Actions</th>}
                        </tr>
                    </thead>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                    <div className="bg-white rounded-lg p-6 w-full max-w-md">
                        ...
                        <form onSubmit={handleSubmit}>
                            ...
                            <input type="text" value={form.nom} onChange={function (e) { setForm({ ...form, nom: e.target.value }); }} className="w-full mb-3 px-3 py-2 border rounded-lg" required />
                            <label className="block text-sm text-gray-600 mb-1">Description</label>
                            <textarea value={form.description} onChange={function (e) { setForm({ ...form, description: e.target.value }); }} className="w-full mb-4 px-3 py-2 border rounded-lg" rows="3" />
